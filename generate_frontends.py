#!/usr/bin/env python3
"""
Generate updated HTML frontend for each show using current episode data
"""
...
<head>
    ...
    <title>{show_title} Episodes</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
    ...
    <script>
        ...
        async function loadEpisodes() {{
            try {{
                ...
                document.getElementById('episodesList').innerHTML = 
                    '<div class="no-results">Error loading episodes. Please try again later.</div>';
            }}
        }}
        ...
        function renderEpisodes(filter = '') {{
            ...
            filtered.forEach((ep) => {{
                ...
                const seasonNum = ep.season !== '?' ? ep.season : '?';
                const episodeNum = ep.episode !== '?' ? ep.episode : '?';
...
def generate_frontends():
    """Generate HTML frontends for all shows"""
    for show_name, show_title in SHOW_TITLES.items():
        show_dir = Path(f'shows/{show_name}')
        html_file = show_dir / 'index.html'
        
        # Generate HTML content
        html_content = HTML_TEMPLATE.format(show_title=show_title)
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("📱 Generating Updated Frontend HTML Files")
    print("="*70 + "\n")
